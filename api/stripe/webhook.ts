...
// Send booking confirmation via WhatsApp (Twilio)
async function sendWhatsAppConfirmation(bookingId: string, companyId: string) {
  try {
    const { data: booking, error } = await supabase
      .from('bookings')
      .select('*, tour_catalog(*)')
    ...
    // Only send if customer provided a phone number
    const phone = booking.customer_phone || booking.phone
    if (!phone) {
      console.log('No customer phone for booking', bookingId)
      return
    }
    ...
    const response = await fetch(`${baseUrl}/api/whatsapp/booking-confirmation`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        to: phone,
    ...
    // WhatsApp failure should not break the webhook
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        await sendWhatsAppConfirmation(bookingId, companyId)
