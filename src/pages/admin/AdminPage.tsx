      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between flex-wrap gap-4">
          ...
          <div className="flex gap-2">
            <button className={`px-4 py-2 rounded-btn text-sm font-600 ${tab === 'overview' ? 'bg-ink text-cream' : 'bg-line text-ink hover:bg-ink hover:text-cream'}`} onClick={() => setTab('overview')}>
              Visão Geral
            </button>
            <button className={`px-4 py-2 rounded-btn text-sm font-600 ${tab === 'invites' ? 'bg-ink text-cream' : 'bg-line text-ink hover:bg-ink hover:text-cream'}`} onClick={() => setTab('invites')}>
              Códigos de Convite
            </button>
            <button className={`px-4 py-2 rounded-btn text-sm font-600 ${tab === 'help' ? 'bg-ink text-cream' : 'bg-line text-ink hover:bg-ink hover:text-cream'}`} onClick={() => setTab('help')}>
              Como Funciona
            </button>
          </div>
        </div>
                    ...
                    <tbody>
                      {activeInvites.map((i) => (
                        <tr key={i.id} className="border-b border-line">
                          ...
                          <td className="py-2 pr-4">
                            <div className="flex gap-2">
                              <button className="px-3 py-1 text-xs rounded-btn bg-yellow text-ink font-600 hover:bg-yellow-dark" onClick={() => copyLink(i)}>
                                {copiedId === i.id ? '✓ Copiado' : 'Copiar link'}
                              </button>
                              <button className="px-3 py-1 text-xs rounded-btn bg-line text-ink2 hover:bg-copper hover:text-cream" onClick={() => handleDeleteInvite(i.id)}>
                                Eliminar
                              </button>
                            </div>
                          </td>
